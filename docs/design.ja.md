...
- HTMLレスポンスではMiddlewareが暗号学的に予測困難なnonceをリクエストごとに生成し、CSPリクエストヘッダーを通じてNext.jsのframework script、page script、インライン初期化scriptへ同じnonceを付与する。本番の`script-src`は`'strict-dynamic'`を使用し、`'unsafe-inline'`と`'unsafe-eval'`を許可しない。nonceの再利用を防ぐためHTMLを動的描画して`Cache-Control: no-store`を設定する。現在のUIは動的なstyle属性を使うため、`style-src`の`'unsafe-inline'`は維持する。APIレスポンスには`default-src 'none'`を基準とするCSPと`Cache-Control: no-store`を適用する。HTMLとAPIの両方でフレーム埋め込み拒否、MIME sniffing拒否、Referrer制御を維持し、HTMLにはPermissions Policyも適用する。
...
- CIでは`LAB_MODE=disabled`と`PUBLIC_SHOWCASE=true`を強制し、依存関係とアプリケーションの検証、OpenNext Workerの1回だけのビルド、Wrangler dry run、workerdへのHTTP境界検証を順に実行する。workerd検証ではHTMLを2回取得し、nonceの一意性、全scriptとの一致、本番`script-src`からの`'unsafe-inline'`・`'unsafe-eval'`除外、公開API遮断を確認する。検証済みの`.open-next`成果物を変更せずdeploy jobへ渡し、Cloudflare認証情報は検証成功後の最終deployステップだけへ公開する。デプロイはリポジトリ設定で明示的に有効化した場合だけ実行する。
