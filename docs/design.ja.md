...
- 開発環境とCIではNode.js 22.12.0以降をサポートし、直接参照するNode.js型定義には対応する`@types/node` 22系を使用する。Dependabotは同型定義のminor・patch更新を引き続き提示するが、サポート対象のruntime majorを意図的に更新して検証するまではmajor更新を除外する。
