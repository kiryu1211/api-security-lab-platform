...
| FR-17 | 公開ショーケース         | 公開環境では日英の学習UIを読み取り専用で表示し、ライブAPI実行を提供しない。                                                 |
...
現行UIは完了状態を保存しません。ローカルラボでは、デモ実行時に脆弱APIと安全APIを並行して呼び出します。公開ショーケースモードでは、実行操作を読み取り専用の案内へ置き換え、APIリクエストを送信しません。
