...
| FR-17 | 公開ショーケース         | 公開環境ではライブAPI実行を停止しつつ、ネットワークリクエストを送信せずに、代表的な合成実行後結果を既存の比較パネルへ表示できる。 |
...
| SR-14 | ブラウザー防御   | CSP、フレーム埋め込み拒否、MIMEスニッフィング防止、リファラー情報の制限、機能制限を共通レスポンスへ適用する。                                                            |
...
- コンポーネントテストで、公開用の合成結果操作が`fetch`を呼び出さずに両方の比較結果を表示することを確認し、静的データテストで全学習テーマを検証する。
...
- 公開用の合成結果操作はライブ実行と明確に区別し、ネットワークへアクセスせずに脆弱側と安全側の両結果を表示する。
