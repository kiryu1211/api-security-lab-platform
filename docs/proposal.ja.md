...
flowchart TD
    ...
    B --> C[リスク概要と実利用場面を確認]
    C --> D[脆弱APIと安全APIをローカルで並行実行]
    D --> E[両方の結果と意味を比較]
    E --> F[設計上の違いを確認]
    F --> G[赤青の実装フロー注釈を確認]
    G --> H[防御策と実装チェックリストを確認]
```

## UI/UX方針

各学習テーマでは、脆弱な実装、安全な実装、リクエスト例、レスポンス例、設計上の違い、実装フロー、防御チェックリストを並べて比較できる画面を用意します。実装フローではAPIプログラム全体の流れを表示し、脆弱な例の問題箇所を赤、安全な例の改善箇所を青で示します。APIデモを実行した後は、レスポンスだけでなく、その結果が何を表すのかも表示します。
