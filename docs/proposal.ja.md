...
Webサービスやモバイルアプリケーションでは、APIが機能提供の中心になることが多くあります。APIはフロントエンド、モバイルアプリ、外部サービスから直接利用されるため、認可不備、認証不備、過剰なデータ公開、レート制限不足、業務フローの悪用、旧API管理の不備、外部API応答の過信などが発生すると、利用者データや業務処理に直接影響します。
...
- 認可、認証、レート制限、業務フロー制御、APIインベントリ管理、入力検証、外部リクエスト制御、外部API応答検証を具体的に検証する。
...
- BOLA、認証とトークン検証、レート制限、Sensitive Business Flows、Mass Assignment、SSRF、Improper Inventory Management、Unsafe Consumption of APIsを学習テーマとして扱う。
...
本システムはアジャイル型で進めることが適しています。BOLA、認証不備、レート制限不足、Sensitive Business Flows、Mass Assignment、SSRF、Improper Inventory Management、Unsafe Consumption of APIsなどの検証テーマを独立したモジュールとして段階的に追加できるためです。
