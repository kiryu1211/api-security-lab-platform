...
- `scripts/verify-local-lab-boundary.mjs` starts the real Next.js development server with local mode enabled on an available port. It requires a successful loopback vulnerable-health response, requires a non-loopback `Host` header to return the disabled error, and confirms at the TCP layer that every available non-loopback IPv4 interface cannot reach the server port. Normal completion, verification failures, and handled termination signals stop the child process and restore Next.js's generated type reference; server output is consumed without publishing local paths or environment details.
