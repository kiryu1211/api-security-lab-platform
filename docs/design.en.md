...
- For HTML responses, Middleware generates a cryptographically unpredictable nonce for every request and passes the CSP through request headers so Next.js applies that nonce to framework scripts, page scripts, and the inline initialization script. Production `script-src` uses `'strict-dynamic'` and permits neither `'unsafe-inline'` nor `'unsafe-eval'`. HTML is dynamically rendered and uses `Cache-Control: no-store` to prevent nonce reuse. Because the current UI uses dynamic style attributes, `style-src 'unsafe-inline'` remains in place. API responses use a CSP based on `default-src 'none'` and `Cache-Control: no-store`. HTML and APIs retain frame denial, MIME-sniffing prevention, and referrer restrictions; HTML also retains Permissions Policy.
...
- CI forces `LAB_MODE=disabled` and `PUBLIC_SHOWCASE=true`, sequentially runs dependency and application checks, builds the OpenNext Worker once, performs a Wrangler dry run, and verifies the public boundary over HTTP in workerd. The workerd check requests HTML twice and verifies nonce uniqueness, nonce coverage across every script, removal of `'unsafe-inline'` and `'unsafe-eval'` from production `script-src`, and the public API shutdown. The verified `.open-next` artifact is passed unchanged to the deploy job. Cloudflare credentials are exposed only to the final deploy step, which runs only after verification and only when explicitly enabled at the repository level.
