...
- Wrangler telemetry is disabled in project configuration for builds, dry runs, previews, and deployments.
...
- Development and CI support Node.js 22.13.0 or later, and direct Node.js declarations use the matching `@types/node` 22 major. Dependabot continues to propose minor and patch updates for those declarations but ignores their major updates until the supported runtime major is deliberately upgraded and verified.
...
- Shared API response helpers are defined in `src/lib/api-response.ts` and return `{ ok, data, meta }` for success or `{ ok, error, meta }` for errors. They are the authoritative API security-header layer for Route Handler and pre-route Middleware responses, applying no-store caching, strict CSP, cross-origin protections, Permissions Policy, referrer and MIME-sniffing controls, and frame denial while removing CORS allow headers.
...
- `src/test-utils/route-inventory.ts` discovers Route Handler files, exported HTTP methods, and dynamic path parameters. Inventory tests require matching secure and vulnerable operations, one local-only guard call per vulnerable operation, exact OpenAPI coverage, and registration in the production-like runtime verification matrix.
- Route inventory uses the TypeScript syntax tree to discover function, variable, and named-re-export HTTP methods. Inventory tests require each POST and PATCH operation to call the shared JSON parser. A dynamic Route Handler matrix sends unsupported content types, oversized declared lengths, exact-limit bodies, and oversized actual bodies to every discovered body-bearing operation and verifies the shared status, error code, route metadata, `no-store`, and MIME-sniffing protection.
- A semantic success-fixture matrix executes all 24 API operations and requires the complete shared API security-header set, JSON response type, route metadata, and absence of CORS allow headers. Existing request-boundary and production-like vulnerable-route matrices apply the same contract to 413, 415, and 403 responses. The local Next.js boundary script verifies secure and vulnerable 200 responses plus Host rejection, while the public workerd script verifies all pre-route API shutdown cases and the absence of `X-Powered-By`.
...
- `scripts/verify-local-lab-boundary.mjs` starts the real Next.js development server with local mode enabled on an available port. It requires protected secure and vulnerable health responses, requires a non-loopback `Host` header to return the protected disabled error, and confirms at the TCP layer that every available non-loopback IPv4 interface cannot reach the server port. Normal completion, verification failures, and handled termination signals stop the child process and restore Next.js's generated type reference; server output is consumed without publishing local paths or environment details.
