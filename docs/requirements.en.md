...
| SR-14 | Browser defenses             | HTML uses a per-request nonce and a production `script-src` that permits neither `'unsafe-inline'` nor `'unsafe-eval'`. APIs use a CSP based on `default-src 'none'`; frame denial, MIME-sniffing prevention, referrer restrictions, and feature restrictions remain in place. |
...
- Tests verify that the HTML nonce changes on every request, every script receives the matching nonce, and production `script-src` contains neither `'unsafe-inline'` nor `'unsafe-eval'`. HTML and API responses retain baseline security headers and both use `no-store`.
