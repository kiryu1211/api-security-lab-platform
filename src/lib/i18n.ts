...
export const uiText = {
  ja: {
    ...
    comparison: {
      ...
      publicShowcaseText:
        "この公開サイトでは安全のためライブAPI実行を停止しています。下のボタンでは通信せずに合成結果を表示できます。実際のAPIデモはローカル環境で実行してください。",
      showSyntheticResults: "合成結果を表示",
      vulnerableResult: "脆弱APIの結果",
      secureResult: "安全APIの結果",
      vulnerableSyntheticResult: "脆弱APIの合成結果",
      secureSyntheticResult: "安全APIの合成結果",
  ...
  en: {
    ...
    comparison: {
      ...
      publicShowcaseText:
        "Live API execution is disabled on this public site for safety. The button below displays synthetic results without making a network request. Run live API demos only in a local environment.",
      showSyntheticResults: "Show synthetic results",
      vulnerableResult: "Vulnerable API result",
      secureResult: "Secure API result",
      vulnerableSyntheticResult: "Synthetic vulnerable API result",
      secureSyntheticResult: "Synthetic secure API result",
