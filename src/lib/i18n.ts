...
export const uiText = {
  ja: {
    ...
    comparison: {
      ...
      publicShowcaseText:
        "この公開サイトでは安全のためライブAPI実行を停止しています。下のボタンではAPIリクエストを送信せず、代表的なリクエスト結果を表示できます。実際のAPIデモはローカル環境で実行してください。",
      showRequestResults: "リクエスト結果を表示",
  ...
  en: {
    ...
    comparison: {
      ...
      publicShowcaseText:
        "Live API execution is disabled on this public site for safety. The button below displays representative request results without sending an API request. Run live API demos only in a local environment.",
      showRequestResults: "Show request results",
