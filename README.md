...
- `npm run test:e2e`: run desktop and mobile Chromium E2E tests against the public-showcase-equivalent environment selected by `PLAYWRIGHT_BASE_URL`. In addition to CSP and API boundaries, the tests verify keyboard operation of primary controls and automated WCAG 2.0, 2.1, and 2.2 A/AA checks with axe. Install the browser once with `npx playwright install chromium`.
