...
公開サイトではライブAPI実行を停止しています。「リクエスト結果を表示」を選択すると、APIリクエストを送信せずに、脆弱側と安全側の代表的な結果をブラウザー内で表示できます。ライブAPIの比較実行はローカル限定です。
...
- 共通APIレスポンス、Zodによるリクエスト検証、ローカルデモ用の合成ユーザーと合成リソースを用意しています。デモ用の主体識別子は、有限個の合成シナリオを選ぶための値であり、認証済み主体ではありません。
...
- 比較画面では、API1からAPI10までの各テーマについて、`/api/vulnerable/*` と `/api/secure/*` のAPI処理全体の流れを表示し、問題箇所を赤、改善箇所を青で確認できます。
...
| API6:2023 Unrestricted Access to Sensitive Business Flows | 重要な業務フローと悪用対策               | `/api/vulnerable/business-flow/reservations` | `/api/secure/business-flow/reservations` |
| API7:2023 Server Side Request Forgery                     | SSRFと外部URL取得制御                    | `/api/vulnerable/fetch-url`                  | `/api/secure/fetch-url`                  |
| API8:2023 Security Misconfiguration                       | セキュリティ設定不備と診断情報の公開制御 | `/api/vulnerable/config/diagnostics`         | `/api/secure/config/diagnostics`         |
...
HTMLとAPIレスポンスには、フレーム埋め込み拒否、Content Security Policy、MIMEスニッフィング防止、リファラー情報の制限、キャッシュ禁止などの共通ヘッダーを適用します。公開ホストへのHTTPリクエストはHTTPSへ恒久的に転送し、HTTPS応答ではHSTSを適用します。HTMLはリクエストごとに新しいnonceを生成してNext.jsのスクリプトとスタイルへ付与します。UIのstyle属性への依存は外部CSSと`data-*`状態へ移しており、本番の`script-src`と`style-src`では`'unsafe-inline'`を許可せず、`script-src`では`'unsafe-eval'`も許可しません。nonce付きHTMLは`no-store`とし、APIには`default-src 'none'`を基準とする厳格なCSPを適用します。JSON本文は `application/json` と任意の `charset=utf-8` だけを受け付け、妥当なUTF-8とJSONを要求し、宣言サイズと実際のバイト数を16 KiB以下に制限します。単一値のクエリパラメーターを重複指定した場合や、strictなクエリスキーマへ未知のパラメーターを指定した場合は、後勝ちで処理せず拒否します。

デモの `userId`、`actorUserId`、トークンIDは、有限の合成シナリオを選択するための値であり、セッションやBearer認証情報ではありません。レート制限バケット、累積予約数、在庫、試行回数は単一プロセス内のデモ用インメモリ状態であり、再起動で失われ、本番環境や分散環境の制御としては使用できません。
...
開発と検証にはNode.js 22.13.0以降が必要です。CIでは利用可能な最新のNode.js 22を使用します。
...
- `npm run security:repository`: 禁止された環境変数ファイル、鍵・証明書、ローカルDB、ログ、開発者専用ファイルのパスと、秘密鍵マーカーがGitの追跡対象に含まれていないことを確認する。
...
- `npm run test:e2e`: `PLAYWRIGHT_BASE_URL`で指定した公開ショーケース相当環境に対し、Chromiumのデスクトップ・モバイルE2Eテストを実行する。CSPとAPI境界に加えて、全10学習テーマの日英表示・合成結果・axe検査、主要操作とオープニングダイアログのキーボード動作、reduced-motion設定時のオープニング省略、WCAG 2.0・2.1・2.2 A/AAの自動検査、代表的なライト・ダーク表示のOS別視覚回帰用基準画像を確認する。初回は`npx playwright install chromium`でブラウザーを導入する。
...
- `npm run deploy:cloudflare`: `production` EnvironmentからWrangler認証情報を渡し、ビルド済みWorkerをデプロイする。デプロイ後は公開URLの境界を再検証する。
...
公開前には、秘密情報、認証情報、非公開ログ、ローカルDB、開発者専用ロードマップがGitの追跡対象に含まれていないことを確認します。CIでは依存関係をインストールする前に`npm run security:repository`で禁止パスと秘密鍵マーカーを検査し、GitHubのsecret scanningとpush protectionで一般的な認証情報パターンを補完します。公開資料では、脆弱なデモがローカル限定であり、公開環境では実行してはいけないことを明記します。CloudflareのAccount IDとAPI Tokenはデプロイ用の秘密情報として扱い、リポジトリ内のファイルには保存しません。セキュリティ・品質検証はpush、Pull Request、手動実行に加えて週次でも実行します。Cloudflareへのデプロイは、リポジトリ設定で明示的に有効化した場合の`main`へのpushだけに限定します。npm依存関係とGitHub ActionsはDependabotの週次Pull Requestで更新候補を確認します。
